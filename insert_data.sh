...
do
  ...
  #add team names to teams table
    ...
    #get team_id if in winner column
    ...
    #get team_id if in opponent column
    OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")
    #if not found, insert opponent
    if [[ -z $OPPONENT_ID ]]
    then
      #insert/search from opponent column
      INSERT_OPPONENT_NAME=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT')")
      OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")
    fi

    #add year, round, winner_id, opponent_id, winner_goals, opponent_goals to games table
